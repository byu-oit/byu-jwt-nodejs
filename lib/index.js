...
// Deprecated: use ByuJWT.prototype.wellKnown instead
// Docs: https://openid.net/specs/openid-connect-discovery-1_0.html#ProviderConfig
...
function ByuJWT (options) {
  ...
  if (!Object.hasOwnProperty.call(options, 'issuer')) options.issuer = options.development ? 'api.byu.edu' : 'api-sandbox.byu.edu'
  ...
  // expose wellKnown url
  byuJwt.wellKnown = `https://${options.issuer}/.well-known/openid-configuration`
  ...
  byuJwt.getOpenIdConfiguration = function () {
    return getOpenIdConfiguration(byuJwt.options.issuer, byuJwt.cache)
  }

  byuJwt.getPem = async function () {
    const keys = await initPem(byuJwt.options.issuer, byuJwt.cache)
 ...
 * @param {object} cache
 */
async function getOpenIdConfiguration (issuer, cache) {
  debug('get OpenID configuration')
  try {
    const config = await request(`https://${issuer}/.well-known/openid-configuration`)
...
async function verifyJWT (options, cache, jwt) {
  ...
  const openIdConfig = await initOpenId(options.issuer, cache)
  const algorithms = openIdConfig.id_token_signing_alg_values_supported

  const validKeys = await initPem(options.issuer, cache)
