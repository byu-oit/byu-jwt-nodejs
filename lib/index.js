...
function ByuJWT(options) {
  ...
  // set cache TTL
  const cache = Cache()
  cache.setTTL(options.cacheTTL)

  const byuJWT = {
    authenticate: headers => init(cache)
      .then(openIdConfig => authenticate(options, openIdConfig, headers)),

    authenticateUAPIMiddleware: (req, res, next) => {
    ...
    decodeJWT: jwt => init(cache)
      .then(openIdConfig => decodeJWT(options, openIdConfig, jwt)),

    getOpenIdConfiguration: () => getOpenIdConfiguration(cache),

    getPublicKey: () => init(cache)
      .then(getPublicKey),

    verifyJWT: jwt => init(cache)
        .then(openIdConfig => verifyJWT(options, openIdConfig, jwt))
        .then(() => true)
        .catch(() => false)
  }
...
function authenticate(options, openIdConfig, headers) {
  ...
  return Promise.all(promises)
    .then(() => {
      ...
      verifiedJWTs.prioritizedClaims = (verifiedJWTs.original && verifiedJWTs.original.resourceOwner) ||
...
function decodeJWT(options, openIdConfig, jwt) {
  return verifyJWT(options, openIdConfig, jwt)
    .then(verifiedJWT => {
      ...
      result.webresCheck = {
        byuId: result[webresCheckKey].byuId,
        netId: result[webresCheckKey].netId,
        personId: result[webresCheckKey].personId
      }
